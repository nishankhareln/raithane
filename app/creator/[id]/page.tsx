...
export default function CreatorProfile() {
  ...
  const following = !!c && !isMe && follows.includes(c.id)

  if (isMe && !user) return (
    <div className="py-20 text-center">
      <p className="text-stone/55">Sign in to see your profile.</p>
      <button onClick={() => openAuth('to see your profile')} className="mt-4 rounded-full bg-clay px-6 py-2.5 text-sm font-black text-white">Sign in</button>
    </div>
  )
  ...
  const skills = isMe
    ? creations.filter(x => x.kind === 'skill').map(toSkill)
    : [...creations.filter(x => x.kind === 'skill' && x.creatorId === c.id).map(toSkill), ...SKILLS.filter(s => s.creatorId === c.id)]
      ...
      {!isMe && (
        <div className="rounded-2xl border-2 border-clay/25 bg-clay/5 p-4">
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2">
            {TIPS.map(t => (
              <button key={t} onClick={() => requireAuth('to support', () => { setTip(t); setCheckout(true) })}
